...
if ($ADMIN->fulltree) {
    ...
    $settings->add(new admin_setting_configselect('mod_streak/cadenceperiod',
        get_string('cadenceperiod', 'mod_streak'), '', 'daily', [
    ...
    $settings->add(new admin_setting_configtext('mod_streak/cadencegoal',
        get_string('cadencegoal', 'mod_streak'), '', 3, PARAM_INT, 4));

    $settings->add(new admin_setting_configtext('mod_streak/freezerate',
        get_string('settings:freezerate', 'mod_streak'), get_string('settings:freezerate_desc', 'mod_streak'),
        4, PARAM_INT, 4));

    $settings->add(new admin_setting_configtext('mod_streak/freezecap',
        get_string('settings:freezecap', 'mod_streak'), '', 2, PARAM_INT, 4));

    $settings->add(new admin_setting_configtext('mod_streak/reminderhour',
        get_string('settings:reminderhour', 'mod_streak'), get_string('settings:reminderhour_desc', 'mod_streak'),
        18, PARAM_INT, 4));
